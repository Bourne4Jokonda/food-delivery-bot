...
DELIVERY_ZONES = {
    ...
    "nearby": {
        ...
        "free_from": 1500,
        "keywords": [
            "пригородное", "борис-глеб", "болтино", "ахидовка", "деревеньки",
            "корцово", "борщево", "козлово", "кутилово", "становое",
            "малышево", "полощиново", "савково", "горяковка", "цепочкино",
            "скрылово", "постнинский", "воронцово", "федьково", "аферьково",
            "иваниха", "горкино", "каменки", "шелково", "коево",
        ],
    },
    "far": {
...
async def handle_delivery_choice(event, vk_id: int, text: str):
    ...
    if "доставка" in text:
        ...
        async with async_session() as session:
            user = await get_or_create_user(vk_id, session)
            if user.phone:
                pending_orders[vk_id]["payment"] = None
                await event.answer("Как будете оплачивать?", keyboard=get_payment_keyboard())
            else:
                pending_orders[vk_id]["awaiting_phone"] = True
...
async def process_order(event, vk_id: int, session: AsyncSession):
    payment_map = {"карта": "card", "наличн": "cash"}
    payment_label = {"card": "💳 Карта", "cash": "💵 Наличные"}
    text = strip_buttons(event.text.strip()).lower()
