...
> **Никогда не отправляйте `.env` третьим лицам, включая ИИ-ассистентов и support-чаты — только `.env.example`.**
> 
> Если ключи были скомпрометированы — перевыпустите их немедленно.
...
### ID чатов сообщества (для уведомлений)
1. Создайте 3 чата в сообществе VK (админы, кухня, курьеры)
2. Добавьте бота в каждый чат
3. Получите ID чата из URL: `https://vk.com/im?sel=2000000123` → ID = `2000000123`
4. Впишите в `.env`:
   ```
   ADMIN_CHAT_ID=2000000123
   KITCHEN_CHAT_ID=2000000456
   COURIER_CHAT_ID=2000000789
   ```
